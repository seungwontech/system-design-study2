...
본 장은 **카파 아키텍처**를 채택한다. 즉, 재계산도 같은 스트리밍 집계 경로를 재사용한다.
...
**워터마크(watermark)**는 윈도 종료 후 짧은 유예 구간을 두어 지연 도착 이벤트를 흡수하는 기법이다.
- 워터마크가 길수록 정확도는 올라가지만 지연 시간이 늘어난다.
- 워터마크가 짧을수록 응답은 빨라지지만 누락 가능성이 커진다.
...
중복이 생기는 대표 시나리오: 집계 노드가 다운스트림 전송 후 업스트림 오프셋 반영 전에 장애가 나면 같은 이벤트를 재소비해 중복 집계한다.
...
책에서 제시한 대안: 클릭 데이터를 Hive에 저장하고, 빠른 질의는 Elasticsearch 계층을 활용하며, 집계는 ClickHouse/Druid 같은 OLAP DB로 처리하는 방식.
