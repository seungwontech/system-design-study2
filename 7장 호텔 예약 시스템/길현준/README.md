...
DB를 선택하기 전에, 시스템이 지원해야 하는 데이터 접근 패턴을 먼저 정리한다.
...
시스템 규모가 크지 않은 것은 추정을 통해 알았으나, 대규모 이벤트가 있는 경우에는 트래픽이 급증할 수 있으므로 대비해야 한다. 이런 요구사항을 종합적으로 고려하여 **관계형 데이터베이스**를 선택한다.
...
낙관적 락은 일반적으로 버전 번호(version number)와 타임스탬프의 두 가지 방법으로 구현한다. 서버 시계는 시간이 지남에 따라 부정확해질 수 있으므로 일반적으로 **버전 번호**를 더 나은 선택지로 본다.

1. **version 열 추가**: 데이터베이스 테이블에 `version`이라는 새 열을 추가한다.
2. **버전 번호 읽기**: 사용자가 데이터베이스 레코드를 수정하기 전에 애플리케이션은 해당 레코드의 버전 번호를 읽는다.
3. **버전 번호 증가 후 기록**: 사용자가 레코드를 갱신할 때 애플리케이션은 버전 번호에 1을 더한 다음 데이터베이스에 다시 기록한다.
4. **유효성 검사**: 다음 버전 번호는 현재 버전 번호보다 정확히 1만큼 큰 값이어야 한다. 이 검사가 실패하면 트랜잭션은 중단(abort)되고, 사용자는 2단계부터 다시 모든 절차를 반복한다.

낙관적 락은 DB에 락을 걸지 않아 비관적 락보다 빠르다. 하지만 동시성 수준이 아주 높으면 성공하는 클라이언트는 오직 하나이고 나머지는 모두 재시도해야 하므로, 반복되는 재시도로 사용자 경험이 나빠질 수 있다.
...
#### 예약 서비스의 잔여 객실 관리 API

캐시 계층이 추가되면 예약 서비스는 다음과 같은 잔여 객실 관리 API를 제공한다.

- **잔여 객실 조회**: 지정된 호텔과 객실 유형, 주어진 날짜 범위에 이용 가능한 객실의 수를 질의
- **객실 예약**: 객실을 예약하고 `total_reserved`의 값을 1 증가
- **예약 취소 처리**: 고객이 예약을 취소하면 잔여 객실 수를 갱신

모든 잔여 객실 관리에 필요한 질의는 Redis로 구현되는 **잔여 객실 캐시**로 옮긴다. 호텔 예약 시스템의 경우 잔여 객실 확인 작업 때문에 읽기 연산 빈도가 쓰기 연산보다 훨씬 많으므로, 대부분의 읽기 연산은 캐시가 처리하고 **잔여 객실 데이터베이스**는 잔여 객실 수에 대한 최종 진실(source of truth)이 보관되는 장소 역할을 한다.
